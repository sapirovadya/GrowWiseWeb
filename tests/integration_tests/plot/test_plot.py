...
def login_as_manager(client):
    with client.session_transaction() as sess:
        ...
        sess["role"] = "manager"
        sess["manager_email"] = "manager@test.com"  # חשוב!



# def test_growth_forecast_basic(client):
...
def test_update_irrigation_basic(client):
    ...
    with patch.object(plot_routes, "db") as mock_db:
        ...
        res = client.post(f"/plots/update_irrigation/{str(plot_id)}", json={
            "irrigation_amount": 3
        })
        assert res.status_code == 200
...
def test_update_plot_quantity_exceeds_stock(client):
    ...
    # מקרה שבו הכמות המבוקשת לשתילה גדולה מהמלאי
    data = {
        "crop": "עגבנייה",
        "crop_category": "ירקות",
        "sow_date": "2024-01-01",
        "quantity_planted": 500,  # כמות גדולה מהמלאי
    }

    with patch.object(plot_routes, "db") as mock_db:
        mock_db.supply.find_one.return_value = {"quantity": 100}  # המלאי הקיים הוא 100
        
        res = client.post(f"/plots/update_plot/{str(plot_id)}", json=data)

        # בדיקת תוצאה
        assert res.status_code == 400  # קוד סטטוס 400 במקרה של כמות גדולה מהמלאי
        response_data = json.loads(res.get_data(as_text=True))  # המרת התגובה ל-JSON
        assert "הזנת כמות גדולה מהמלאי" in response_data.get("error")  # בדיקה שההודעה נכונה
...
def test_update_plot_success(client):
    ...
    with patch.object(plot_routes, "db") as mock_db:
        # mock למצב שבו חלקה קיימת ונתוני המלאי מסודרים
...
def test_update_plot_not_found(client):
    ...
    with patch.object(plot_routes, "db") as mock_db:
        mock_db.plots.find_one.return_value = None  # החלקה לא קיימת
